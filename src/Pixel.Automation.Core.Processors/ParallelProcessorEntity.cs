...
namespace Pixel.Automation.Core.Processors
{
    ...
    public class ParallelProcessorEntity : BaseProcessorEntity
    {
       
        public ParallelProcessorEntity():base("Parallel Processor", "Processor")
        {
        ...
        public override Task BeginProcess()
        {
            ...
            Task processorTask = new Task(() =>
            {
                ...
                var result = Parallel.ForEach(this.Components, async parallelComponent =>
                {
                    IComponent actorBeingProcessed = null;
                    try
                    {
                        parallelComponent.BeforeProcess();
                        foreach (var component in (parallelComponent as Entity).GetNextComponentToProcess())
                        {
                        ...
                        parallelComponent.OnCompletion();
                    ...
                    catch (Exception ex)
                    {
                        ...
                        parallelComponent.OnFault(actorBeingProcessed);
                    }
                    finally
                    {
