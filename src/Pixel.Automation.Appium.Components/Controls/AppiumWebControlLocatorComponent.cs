...
/// <summary>
/// Implementation of <see cref="IControlLocator"/> and <see cref="ICoordinateProvider"/> to locate a web control in a browser or webview context using appium.
/// </summary>
[DataContract]
[Serializable]
[ToolBoxItem("Appium Web Locator", "Control Locators", iconSource: null, description: "Identify a web control on screen", tags: new string[] { "Locator" })]
public class AppiumWebControlLocatorComponent : ServiceComponent, IControlLocator, ICoordinateProvider
{
    private readonly ILogger logger = Log.ForContext<AppiumWebControlLocatorComponent>();
    ...
    public AppiumApplication ApplicationDetails
    {
        get
        {
            return EntityManager.GetOwnerApplication<AppiumApplication>(this);
        }
    }
    ...
    IWebDriver WebDriver
    {
        get
        {
            return ApplicationDetails.Driver;
        }
    }
    ...
    /// Constructor      
    /// </summary>
    public AppiumWebControlLocatorComponent() : base("Appium Web Control Locator", "AppiumWebControlLocator")
    {
    ...
    {
        return controlIdentity is AppiumWebControlIdentity;
    }
    ...
    {
        Guard.Argument(controlIdentity).Compatible<AppiumWebControlIdentity>();

        var webControlIdentity = controlIdentity as AppiumWebControlIdentity;
    ...
    private ISearchContext FindCurrentControl(AppiumWebControlIdentity webControlIdentity, ISearchContext currentRoot)
    {
    ...
    {
        Guard.Argument(controlIdentity).NotNull().Compatible<AppiumWebControlIdentity>();
        ISearchContext currentRoot = searchRoot?.GetApiControl<ISearchContext>() ?? WebDriver;
    ...
    internal bool HasShadowRoot(ISearchContext searchContext)
    {
        ...
        var result = (WebDriver as IJavaScriptExecutor).ExecuteScript("return (arguments[0].shadowRoot != null)", arguments.ToArray());
        return (bool)result;
    }

    #region Descendant Control

    internal IWebElement FindDescendantControl(AppiumWebControlIdentity controlIdentity, ISearchContext searchRoot)
    {
    ...
    internal IReadOnlyCollection<IWebElement> FindAllDescendantControls(AppiumWebControlIdentity controlIdentity, ISearchContext searchRoot = null)
    {
        Guard.Argument(controlIdentity).NotNull(); ;
    ...
    internal IWebElement FindAncestorControl(AppiumWebControlIdentity controlIdentity, ISearchContext searchRoot)
    {
        Guard.Argument(controlIdentity).NotNull();
        ConfigureRetryPolicy(controlIdentity);
        IWebElement foundControl = default;
        try
        {
            AppiumWebControlIdentity webControlIdentity = controlIdentity as AppiumWebControlIdentity;
                ...
                case "TagName":
                    ...
                    foundControl = policy.Execute(() =>
                    {
                        ...
                        return new WebElementWait(searchRoot, TimeSpan.FromSeconds(searchTimeout)).
                         Until(p => { return jsExecutor.ExecuteScript(ancestorLookupScript, searchRoot, ancestorSelector) as IWebElement; });
                    });
                    break;
                case "XPath":
    ...
    internal IWebElement FindSiblingControl(AppiumWebControlIdentity controlIdentity, ISearchContext searchRoot)
    {
        Guard.Argument(controlIdentity).NotNull();
    ...
    internal IReadOnlyCollection<IWebElement> FindAllSiblingControls(AppiumWebControlIdentity controlIdentity, ISearchContext searchRoot = null)
    {
        Guard.Argument(controlIdentity).NotNull();
        ...
        switch (webControlIdentity.FindByStrategy)
        {
            ...
            case "TagName":
                var foundControls = policy.Execute(() =>
                {
                    ...
                    Until(p =>
                    {
                        return jsExecutor.ExecuteScript(siblingLookupScript, searchRoot, siblingSelector)
    ...
    private IWebElement FindElement(AppiumWebControlIdentity webControlIdentity, ISearchContext searchRoot)
    {
        IWebElement foundControl = default;
    ...
    private ReadOnlyCollection<IWebElement> FindAllElement(AppiumWebControlIdentity webControlIdentity, ISearchContext searchRoot)
    {
    ...
    {
        AppiumWebControlIdentity controlIdentity = controlDetails as AppiumWebControlIdentity;
    ...
    public void SwitchToFrame(FrameIdentity frame, AppiumWebControlIdentity webControlIdentity)
    {
        IWebDriver webDriver = WebDriver;
        ...
        switch (findBy)
        {
            ...
            case "CssSelector":
            case "XPath":
                IWebElement frameElement = FindDescendantControl(new AppiumWebControlIdentity()
                {
    ...
    protected IWebElement GetWebElementAtConfiguredIndex(IReadOnlyCollection<IWebElement> foundControls, AppiumWebControlIdentity webControlIdentity)
    {
    ...
    private void SwitchToTargetFrame(AppiumWebControlIdentity webControlIdentity)
    {
    ...
    private void HighlightElement(IWebElement foundControl)
    {
        if (ShowBoundingBox && foundControl != null)
        {
            ...
            {
                highlightRectangle = EntityManager.GetServiceOfType<IHighlightRectangle>();
            }
    ...
    {
        if (retryAttempts != controlIdentity.RetryAttempts || retryInterval != controlIdentity.RetryInterval)
        {
