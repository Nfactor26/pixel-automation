...
{
    public class WhileLoopEntityTest
    {
        /// <summary>
        /// Vadidate that While loop entity runs until specified condition is true
        /// </summary>
        [Test]
        public void ValidateThatWhileLoopEntityLoopsUntilConditionStaysSatisfied()
        {
            var entityManager = Substitute.For<IEntityManager>();        

            var scriptEngine = Substitute.For<IScriptEngine>();
            scriptEngine.ExecuteFileAsync(Arg.Any<string>()).Returns(new ScriptResult(true), new ScriptResult(true), new ScriptResult(false));
            ...
            scriptEngine.Received(3).ExecuteFileAsync(Arg.Any<string>());
        }
