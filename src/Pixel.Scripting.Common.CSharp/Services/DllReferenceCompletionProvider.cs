...
{    
    [ExportCompletionProvider(nameof(DllReferenceCompletionProvider), LanguageNames.CSharp)]
    public class DllReferenceCompletionProvider : CompletionProvider
    {
            ...
            {                
                foreach (var directory in Enumerable.Concat(new[] { AppContext.BaseDirectory }, Directory.GetDirectories(Path.Join(AppContext.BaseDirectory, "Plugins"))))
                {
                    ...
                    foreach (var dll in dlls)
                    {
                        ...
                        completionItem = completionItem.WithRules(s_rules);
