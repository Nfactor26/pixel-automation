...
[DataContract]
[Serializable]
[ToolBoxItem("Get Directories", "System.IO", "Directory", iconSource: null, description: "Returns the names of subdirectories that meet specified criteria", tags: new string[] { "Get Directories" })]
public class GetDirectoriesActorComponent : ActorComponent
{
    ...
    [DataMember]
    [DisplayName("Directories Result")]
    [Category("Output")]
    [Description("Collection of the full names (including paths) of the subdirectories that match the search pattern and enumeration options in the specified directory, or an empty array if no directories are found.")]
    public Argument DirectoriesResult { get; set; } = new InArgument<IEnumerable<string>>() { Mode = ArgumentMode.DataBound };

    public GetDirectoriesActorComponent() : base("Get Directories", "GetDirectories")
    {
    ...
    public override async Task ActAsync()
    {
        ...
        if (this.SearchPattern.IsConfigured())
        {
            ...
            if(this.EnumerationOptions.IsConfigured())
            {
                ...
                directories.AddRange(Directory.GetDirectories(path, searchPattern, options));
        ...
        {
            await argumentProcessor.SetValueAsync(this.DirectoriesResult, result);
            logger.Information("{0} Directories found at Path : {1}", result.Count(), path);
        }
    }
}
