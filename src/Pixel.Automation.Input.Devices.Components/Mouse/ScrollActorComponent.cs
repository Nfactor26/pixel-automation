    ...
    public override async Task ActAsync()
    {
        ...
        switch (this.ScrollDirection)
        {
            ...
            case ScrollDirection.Right:
                ...
                logger.Information("Mouse was horizontally scrolled by {0} ticks in direction {1}", amountToScroll, this.ScrollDirection);
                break;
            case ScrollDirection.Up:
            case ScrollDirection.Down:
                syntheticMouse.VerticalScroll(amountToScroll);
                logger.Information("Mouse was vertically scrolled by {0} ticks in direction {1}", amountToScroll, this.ScrollDirection);
                break;
        }
